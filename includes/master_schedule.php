...
function master_schedule($atts) {
	...
	$show_djs = $attr['show_djs'];
	$divheight = $attr['divheight'];
	$single_day = $attr['single_day'];
	...
	if($list == 1 || $list == 'list') {
		...
		foreach($flip as $day => $hours) {
			...
			//if we're only display a schedule for a specific day, output that day and skip everything else
			if($single_day != '' && strtolower($single_day) != strtolower($day)) {
				continue;
			}
