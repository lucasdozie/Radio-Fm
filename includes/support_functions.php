	...
	foreach($show_shifts as $shift) {
		...
		foreach($shift->meta_value as $time) {
			...
			if($time['day'] == $curDay) {
				...
				//echo $notice."display Day - ".$time['day']." </br>";
				//compare to the current timestamp
				if($time['start_timestamp'] <= $now && $time['end_timestamp'] >= $now) {
					$show_ids[] = $shift->post_id;

					//echo $notice ."Compare curt date ".$time['start_timestamp']." <= ".$now." => ".$time['end_timestamp']." </br>";
			...
			if( date('w', strtotime($time['day']))+1 == date('w', strtotime($curDay)) ) {
				...
				//echo $notice ."special allow ".$time['start_timestamp']." <= ".$now." => ".$time['end_timestamp']." </br>";
				//compare to the current timestamp
				if($time['start_timestamp'] <= $now && $time['end_timestamp'] >= $now) {
					$show_ids[] = $shift->post_id;
					//echo $notice."compare current timestamp again".$time['start_timestamp']."<= ".$now." => ".$time['end_timestamp'];
				}
			}
